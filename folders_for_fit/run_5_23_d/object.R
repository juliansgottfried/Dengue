rproc <- Csnippet("
    ...
    W += (P-kappa*pow(25+T,2)*(100-RH))/W_max;
    if (W>1) W=1;
    else if (W<0) W=0;
    
    hill = (pow(W,n1+1)/(pow(W,n1+1)+pow(K_a,n1+1)))*(pow(K_b,n2+1)/(pow(K_b,n2+1)+pow(W,n2+1)));
    k = hill*exp(theta+bNino*nino+bNina*nina);
    foi = (beta_out+k*beta_eff*pow(I/pop,alpha))*(noise/dt);
...
covars <- c("time","pop","dpopdt","beta_eff","P","T","RH","nino","nina")
